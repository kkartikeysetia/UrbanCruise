...
function OfferSection({
    ...
    return (
      <Col>
        <p
          ...
          style={{ color: theme === "dark" ? "#ADD8E6" : "#6c757d" }}
        >
  ...
  const itemsToDisplay =
    maxItemsToShow !== undefined && maxItemsToShow !== null
        ...
        {itemsToDisplay.map(([key, value]) => {
          ...
          if (
            ...
            if (foundModelInState) {
              ...
              console.warn(
                `WARN (OfferSection): Model ID '${originalModelId}' not found for brand '${brand}' (prefixed key: '${prefixedBrandKey}'). Available models for brand:`,
                brandModels
              );
            }
          } else {
            console.warn(
              `WARN (OfferSection): No models object found for prefixed brand key: '${prefixedBrandKey}'. Active category: ${activeCategory}, Brand ID: ${originalBrandId}. Full models state:`,
              models
            );
          }
            ...
            >
              ...
              >
                ...
                <div className="gallery-content text-center px-2 py-3 d-flex flex-column flex-grow-1 justify-content-between">
                  <div>
                    <h3 className="fs-4 fw-600" style={{ color: textColor }}>
                      {brand}
                    </h3>
                    <p
                      className="fs-5 fw-500 mb-2"
                      style={{
                        color:
                          theme === "dark"
                            ? primaryColorDark
                  ...
                  <div className="d-grid mt-auto">
                    <Link
                      to={`/${activeCategory}s/${urlBrand}/${urlModel}/${key}`}
                    >
                      <Button
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (fetchedCarBrandsSnap.exists()) {
          ...
          console.log(
            "Raw car brands data from 'vehicle/brands' document:",
            brandsData
          );
          if (
            brandsData &&
            ...
            console.log(
              "Successfully processed car brands. Added to allFetchedBrands."
            );
          } else {
            console.warn(
              "WARN (CarOffers FetchData - Car Brands): 'brands' field not found in 'vehicle/brands' document or is not a valid object.",
              fetchedCarBrandsSnap.data()
            );
          }
        } else {
          console.warn(
            "WARN (CarOffers FetchData - Car Brands): 'vehicle/brands' document does not exist. Check document ID."
          );
        }
        ...
        if (fetchedBikeBrandsSnap.exists()) {
          ...
          console.log(
            "Raw bike brands data from 'vehicle/bike-brands' document:",
            bikeBrandsData
          );
          if (
            bikeBrandsData &&
            typeof bikeBrandsData === "object" &&
            bikeBrandsData !== null
          ) {
            ...
            console.log(
              "Successfully processed bike brands. Added to allFetchedBrands."
            );
          } else {
            console.warn(
              "WARN (CarOffers FetchData - Bike Brands): 'brands' field not found in 'vehicle/bike-brands' document or is not a valid object.",
              fetchedBikeBrandsSnap.data()
            );
          }
        } else {
          console.warn(
            "WARN (CarOffers FetchData - Bike Brands): 'vehicle/bike-brands' document does not exist. Check document ID."
          );
        }
        setBrands(allFetchedBrands);
        console.log(
          "Final 'brands' state after all brand processing:",
          allFetchedBrands
        );
        ...
        const processModelsData = (snap, categoryPrefix, targetObj) => {
          if (snap.exists()) {
            ...
            console.log(
              `Raw '${categoryPrefix}-models' document data (field 'models'):`,
            ...
            if (
              ...
              itemsToIterate.forEach((brandModelGroup) => {
                if (
                  ...
                  if (
                    typeof modelsContent === "object" &&
                    modelsContent !== null
                  ) {
                    processedModelsForBrand = Object.fromEntries(
                      Object.entries(modelsContent).map(([id, name]) => [
                        String(id),
                        name,
                      ])
                    );
                    ...
                    console.warn(
                      `WARN (CarOffers FetchData - ${categoryPrefix} models): 'models' content for brandId '${actualBrandId}' is not an object (map). Actual type: ${typeof modelsContent}. Value:`,
                      modelsContent
                    );
                  }
                  ...
                  console.log(
                    `Processed models for ${categoryPrefix} brand ${actualBrandId}:`,
                    processedModelsForBrand
                  );
                } else {
                  console.warn(
                    `WARN (CarOffers FetchData - ${categoryPrefix} models): Invalid brandModelGroup structure in '${categoryPrefix}-models' document: missing brandId or models field, or not an object.`,
                    brandModelGroup
                  );
                }
              });
              ...
              console.warn(
                `WARN (CarOffers FetchData - Models for ${categoryPrefix}): 'models' field within 'vehicle/${categoryPrefix}-models' document is not an object/array or is missing. Actual value:`,
                rawModelsForCategory
              );
            }
          } else {
            console.warn(
              `WARN (CarOffers FetchData - Models for ${categoryPrefix}): 'vehicle/${categoryPrefix}-models' document does not exist. This is required for models.`
            );
          }
        };
        ...
        console.log(
          "Final 'models' state after all model processing:",
          combinedModels
        );
  ...
  const filteredItems = vehiclesForCurrentCategory
    ? Object.entries(vehiclesForCurrentCategory).filter(([id, v]) => {
        ...
        if (appliedBodyType && appliedBodyType !== "All") {
          const vehicleBodyType = v.bodyType
            ? String(v.bodyType).toLowerCase()
            : "n/a";
        ...
        if (
          ...
          const vehicleFuelType = v.fuelType
            ? String(v.fuelType).toLowerCase()
            : "n/a";
  ...
  return (
    <div
      ...
      style={{
        ...
        borderBottom:
          theme === "dark" ? "1px solid #0A0A0A" : "1px solid #F8F8F8", // Match background color
        borderLeft:
          theme === "dark" ? "1px solid #0A0A0A" : "1px solid #F8F8F8", // Match background color
        borderRight:
          theme === "dark" ? "1px solid #0A0A0A" : "1px solid #F8F8F8", // Match background color
            ...
            <h1
              ...
              style={{
                ...
                backgroundImage: `linear-gradient(45deg, ${
                  theme === "dark" ? "#33D9B2" : "#FF6347"
                }, ${theme === "dark" ? "#20C997" : "#CC4C3B"}, ${
                  theme === "dark" ? "#1ABC9C" : "#FF6347"
                })`,
            ...
            >
              <div
                ...
                style={{
                  flexShrink: 0,
                  border:
                    theme === "dark" ? "1px solid #444" : "1px solid #D0D0D0",
                  borderRadius: "10px",
                  ...
                  scrollbarWidth: "thin",
                  scrollbarColor: `${theme === "dark" ? "#555" : "#888"} ${
                    theme === "dark" ? "#333" : "#f1f1f1"
                  }`,
                }}
              >
                {vehicleCategories
                  .slice(0, MAX_VISIBLE_TABS_MOBILE)
                  .map((category, index) => (
                    <Button
                      key={category.value}
                      ...
                      style={{
                        ...
                        borderRight:
                          index <
                          vehicleCategories.slice(0, MAX_VISIBLE_TABS_MOBILE)
                            .length -
                            1
                            ? theme === "dark"
                              ? "1px solid #3A3A3A"
                        ...
                        ...(index === MAX_VISIBLE_TABS_MOBILE - 1 &&
            ...
            {showFilterDropdown && (
              <div
                style={{
                  ...
                  border:
                    theme === "dark" ? "1px solid #666" : "1px solid #ccc",
                  borderRadius: "10px",
                  padding: "1.5rem",
              ...
              >
                <h4
                  ...
                  style={{ color: theme === "dark" ? "#20C997" : "#007bff" }}
                >
                  Filter Options
                </h4>
                ...
                <Form.Group className="mb-4">
                  <Form.Label
                    ...
                    style={{ color: theme === "dark" ? "#E0E0E0" : "#444" }}
                  >
                    {activeType === "car"
                      ? "Car Body Type"
                      : "Vehicle Body Type"}
                ...
                <div
                  ...
                  style={{ borderColor: theme === "dark" ? "#444" : "#eee" }}
                >
                  <Button
                    variant={
                      theme === "dark" ? "outline-info" : "outline-secondary"
                    }
        ...
        {isLoading ? (
          <div className="text-center py-5">
            ...
            <p
              className="mt-3"
              style={{
                color: theme === "dark" ? "#ADD8E6" : "var(--bs-primary)",
              }}
            >
              Loading {activeType} offers...
            </p>
          </div>
        ) : fetchError ? (
          ...
          <p
            ...
            style={{ color: theme === "dark" ? "#ADD8E6" : "#6c757d" }}
          >
            No {activeType}s available with these filters. Try adjusting your
            filters.
          </p>
        )}
      </Container>
